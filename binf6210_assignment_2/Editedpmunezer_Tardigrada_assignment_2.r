...
## 
## 2025-10-13
## Secondary contributor: Maureen Mabel Angelus
##Implemented five edits — (1) included the janitor package for clean up of column names (2) excluded "Unrecoverable" from curve input, (3) added Simpson diversity alongside Shannon and Evenness, (4) added species vs BIN richness diagnostic plot, and (5) modified a minor parameter typo conserving the original intention of the code.
##Pull Request reviewed by primary author.
## edits made submitted on November 13/2025.
##*******************************
##
...
library(VennDiagram)
library(ggpubr)
library(janitor)  # EDIT: to clean names
...
#EDIT: using janitor to clean names instead of manually having to
df_tardi <- janitor::clean_names(df_tardi)  
...
#EDIT: Since we have documented that "Unrecoverable" is large, we can exclude it so it does not confound the results of this curve.
df_tardi.curve <- df_tardi %>%
  filter(country_ocean != "Unrecoverable" | is.na(country_ocean))

## Let's remove missing values before grouping
df_tardi.BINs.by.country <- df_tardi.curve %>% 
...
#EDIT: It could be useful to incorporate a visualization of how BIN richness compares to Species richness to verify that they correlate strongly

ggplot(df_tardi.countries.sub, aes(x = species_richness, y = bin_richness)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, linetype = "dashed") +
  labs(title = "Species richness vs BIN richness per country",
       x = "Unique species", y = "Unique BINs")
...
#EDIT: Simpson index may also be added to provide additional, complimentary information
df_tardi.countries_shannon_evenness <- df_tardi.countries %>% 
  filter(!is.na(country_ocean) & !is.na(bin_uri)) %>% 
  group_by(country_ocean, bin_uri) %>% 
  summarize(count = n(), .groups = "drop") %>% 
  group_by(country_ocean) %>% 
  summarize(shannon = diversity(count, index = "shannon"), simpson = diversity(count, index = "simpson"), richness = n_distinct(bin_uri), evenness = ifelse(richness > 1, shannon / log(richness), NA), total_records = sum(count)) %>% 
  arrange(desc(shannon)) %>%
  print()
...
# Let's see skewness with density plot 
ggplot(data = df_tardi.countries_shannon_evenness.clean) +
...
# Draw the diagram
# Let's first create a new page for the diagram
grid.newpage()

# Let's dram the diagram using the grid object
grid.draw(df_tardi.venn_plot)
grid.text("Overlap and unique BINs in well-sampled and underrepresented regions", x = unit(0.5, "npc"), y = unit(0.95, "npc"), gp = gpar(fontsize = 12.5))
# EDIT: There was a minor typo in the font size specified; where "fontosize=40" was specified. Since that parameter is not recognized, R seems to have defaulted to the baseline setting. I modified it to look a little bigger to follow the intention behind the original script 
